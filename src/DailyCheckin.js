  ...
  return (
    <div style={{ background: "rgba(0,0,0,0.2)", borderRadius: 20, padding: 18, marginBottom: 12 }}>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <TrafficLight value={isDone ? checkin.nutrition_score : nutrition} onChange={setNutrition} disabled={isDone || !isToday} />
      </div>

      {/* Savijauta */}
      <div style={{ marginBottom: isDone ? 0 : 16 }}>
        <p style={{ fontSize: 12, color: "rgba(255,255,255,0.7)", fontWeight: 600, margin: "0 0 8px" }}>💚 Kokia šiandien savijauta?</p>
        <TrafficLight value={isDone ? checkin.wellbeing_score : wellbeing} onChange={setWellbeing} disabled={isDone || !isToday} />
      </div>
