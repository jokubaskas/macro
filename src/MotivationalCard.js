...
async function analyzeAndGenerateMessage(userId, goalId) {
  ...
  if (positives.length >= 2 && negatives.length === 0) {
    return { emoji: "🌟", main: pick(["Fantastiška savaitė! Vanduo, miegas, mityba – viskas top! 🌟", "Puiki savaitė! Matosi pastangos – ir jos tikrai vertos! ✨"]), tip: positives[0]?.msg, science: positives[0]?.science || SCIENCE.general, type: "great" };
  }
  if (positives.length > 0 && negatives.length > 0) {
    return { emoji: "✨", main: positives[0].msg, tip: "Atkreipk dėmesį: " + negatives[0].msg, science: negatives[0].science, type: "good" };
  }
  if (negatives.length > 0) {
    return { emoji: "💙", main: pick(["Sunkesnė savaitė – bet tai tik informacija, ne nesėkmė. Rytoj nauja proga! 💙", "Kiekviena diena – nauja pradžia. Nesibark, tiesiog žerk kitą žingsnį 💪"]), tip: negatives[0].msg, science: negatives[0].science, type: "encourage" };
  }
  return { emoji: "😊", main: positives[0]?.msg || "Taip laikykis! 💪", tip: null, science: positives[0]?.science || SCIENCE.general, type: "neutral" };
}
...
export default function MotivationalCard({ userId, goalId }) {
  ...
  return (
    <div style={{ background: "rgba(0,0,0,0.2)", borderRadius: 20, padding: "16px 18px", marginBottom: 12, border: "1px solid rgba(255,255,255,0.15)" }}>
      ...
      {msg.science && (
        <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: 12, padding: "9px 12px", borderLeft: "2.5px solid rgba(255,255,200,0.3)", marginTop: 4 }}>
