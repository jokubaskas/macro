    ...
    <div style={{ position:"fixed", inset:0, zIndex:500, background:"linear-gradient(160deg,#3a0a20 0%,#6D1B3B 45%,#AD1457 100%)", overflowY:"auto", paddingBottom:80, WebkitOverflowScrolling:"touch", fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif", animation:"fadeInUp 0.32s cubic-bezier(.23,1,.32,1) both" }}>
      ...
      <div style={{ maxWidth:480, margin:"0 auto", padding:16 }}>
        ...
        {pending.map(p => (
          <div key={p.id} style={{ background:"rgba(255,200,0,0.08)", border:"1px solid rgba(255,200,0,0.25)", borderRadius:14, padding:"12px 16px", marginBottom:10, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <div>
              ...
              <p style={{ fontSize:11, color:"#FFD700", margin:0, display:"flex", alignItems:"center", gap:4 }}><Timer size={11} />Laukia patvirtinimo</p>
            </div>
          </div>
        ))}
        ...
        <div style={{ background:"rgba(255,255,255,0.06)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:14, padding:"14px 16px", marginTop:8 }}>
          <p style={{ fontSize:12, color:"rgba(255,255,255,0.7)", margin:"0 0 8px", lineHeight:1.5, display:"flex", alignItems:"center", gap:6 }}><CreditCard size={13} />Išsiuntus užklausą, atlikite apmokėjimą ir trenerė patvirtins jūsų paketą.</p>
