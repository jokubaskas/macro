...
export default function WaterTracker({ goal: defaultGoal = 2000, userId, date, compact = false }) {
  ...
  useEffect(() => {
    ...
    setLoaded(false);
    async function load() {
      const { data } = await supabase.from("water_log").select("ml, goal").eq("user_id",userId).eq("date",currentDate).maybeSingle();
  ...
  async function save(newDrunk) {
    ...
    setSaving(true);
    await supabase.rpc("upsert_water_log", { p_user_id:userId, p_date:currentDate, p_ml:newDrunk, p_goal:goal });
    setSaving(false);
  }
  ...
  function clickGlass(index) {
    ...
    setDrunk(newDrunk);
    save(newDrunk);
    // FIX: auto-scroll pašalintas — neslinkti automatiškai
  }
  ...
  const GlassSVG = ({ index }) => {
    ...
    return (
      <div onClick={() => clickable && clickGlass(index)}
        style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:4, cursor:clickable?"pointer":"default", flexShrink:0, width:52 }}>
        <svg viewBox="0 0 48 60" width="42" height="52">
          ...
          <rect x="6" y="34" width="36" height="22"
            ...
            clipPath={"url(#wcp"+index+")"} style={{ transition:"all 0.3s" }} />
  ...
  return (
    <div style={{ background:"rgba(0,0,0,0.2)", borderRadius:20, padding:"16px 16px", boxShadow:"0 6px 24px rgba(173,20,87,0.3)" }}>
      ...
      <div style={{ background:"rgba(255,255,255,0.2)", borderRadius:99, height:5, marginBottom:14 }}>
        <div style={{ width:pct+"%", height:"100%", borderRadius:99, background:done?"#7FFFB0":"rgba(255,255,255,0.85)", transition:"width 0.4s cubic-bezier(.23,1,.32,1)" }}/>
      </div>

      {/* FIX: scrollRef išlikęs, bet auto-scroll logika pašalinta */}
      <div ref={scrollRef} style={{ display:"flex", gap:6, overflowX:"auto", paddingBottom:4, scrollbarWidth:"none", msOverflowStyle:"none", WebkitOverflowScrolling:"touch" }}>
        {Array.from({ length: totalGlasses }, (_, i) => <GlassSVG key={i} index={i} />)}
        {isToday && (
          <div onClick={addGlass} style={{ flexShrink:0, width:52, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:4, cursor:"pointer" }}>
            <div style={{ width:42, height:52, borderRadius:10, border:"1.5px dashed rgba(255,255,255,0.35)", display:"flex", alignItems:"center", justifyContent:"center" }}>
