...
export default function WaterTracker({ goal: defaultGoal = 2000, userId }) {
  ...
  async function save(newDrunk, newGoal) {
    ...
    await supabase.from("water_log").upsert(
      { user_id: userId, date: todayStr(), ml: newDrunk, goal: newGoal },
      { onConflict: "user_id,date" }
    );
    setSaving(false);
  }

  function clickGlass(index, isFilled) {
    const newDrunk = isFilled ? Math.max(0, index * ML) : (index + 1) * ML;
    setDrunk(newDrunk);
    save(newDrunk, goal);
  }
  ...
  const Glass = ({ index }) => {
    ...
    return (
      <div onClick={() => clickGlass(index, isFilled)}
        ...
        <svg viewBox="0 0 48 64" width="44" height="58" style={{ display:"block" }}>
          ...
          {isFilled && (
            <text x="24" y="30" textAnchor="middle" fontSize="14"
              fill="#fff" fontWeight="700" opacity="0.8">v</text>
          )}
        </svg>
        <span style={{ fontSize:10, color:isFilled ? PK.mid : PK.rose, fontWeight:isFilled?"700":"400" }}>
          250ml
        </span>
      </div>
    );
  ...
  return (
    <div style={{
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:14 }}>
        <div>
          ...
          <div style={{ display:"flex", alignItems:"baseline", gap:6 }}>
            <span style={{ fontSize:28, fontWeight:700, color:done?"#1D9E75":PK.water }}>{drunk}</span>
      ...
      <div style={{
        ...
        gap:8, marginBottom:14, justifyItems:"center",
      }}>
        {Array.from({ length: totalGlasses }, (_, i) => <Glass key={i} index={i} />)}
      </div>

      <div style={{ display:"flex", gap:8 }}>
        <button onClick={addGlass} style={{
          flex:1, padding:"11px 0", background:PK.light,
          border:"1px solid "+PK.blush, borderRadius:12,
          fontSize:13, fontWeight:700, color:PK.mid,
          cursor:"pointer", fontFamily:"inherit",
        }}>+ Stikline (250ml)</button>
        <button onClick={reset} style={{
          padding:"11px 14px", background:"#fff",
          border:"1px solid "+PK.blush, borderRadius:12,
          fontSize:13, color:PK.rose, cursor:"pointer", fontFamily:"inherit",
        }}>Istrinti</button>
      </div>

      <p style={{
        margin:"10px 0 0", fontSize:12, textAlign:"center",
        color:done?"#1D9E75":PK.rose, fontWeight:done?700:400,
      }}>
        {drunk === 0
          ? "Spausk ant stiklines norėdamas pradeti"
          : done
          ? "Tikslas pasiektas! Puiku!"
          : "Liko "+remaining+"ml ("+Math.ceil(remaining/ML)+" stiklines)"}
      </p>
    </div>
  );
}
