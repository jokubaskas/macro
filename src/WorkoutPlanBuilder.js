...
import { pb } from "./pb";
import { EXERCISES, MUSCLE_GROUPS } from "./exerciseDatabase";
    ...
    <div style={{ position:"fixed", inset:0, zIndex:600, background:"rgba(0,0,0,0.75)", display:"flex", alignItems:"flex-end" }}
      ...
      <div style={{ width:"100%", maxWidth:480, margin:"0 auto", background:"linear-gradient(160deg,#2d0a1a,#6D1B3B)", borderRadius:"24px 24px 0 0", padding:"20px 16px 40px", maxHeight:"90vh", overflowY:"auto" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
          <p style={{ fontSize:15, fontWeight:700, color:"#fff", margin:0 }}>Pasirinkti pratimą</p>
        ...
        {!selected ? (
          <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
            {filtered.map(ex => (
              <button key={ex.name} onClick={()=>setSelected(ex)} style={{ padding:"11px 14px", background:"rgba(255,255,255,0.08)", border:"1px solid rgba(255,255,255,0.15)", borderRadius:12, color:"#fff", cursor:"pointer", fontFamily:"inherit", textAlign:"left", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                <div>
...
export default function WorkoutPlanBuilder({ client, onClose, onSaved }) {
  const [step, setStep]           = useState(1); // 1=parametrai, 2=pratimų sudėliojimas
  const [daysCount, setDaysCount] = useState(3);
  const [validUntil, setValidUntil] = useState("");
  const [planName, setPlanName]   = useState("");
  const [days, setDays]           = useState([]); // [{ day_number, day_name, exercises:[] }]
  ...
  function initDays(count) {
    setDays(Array.from({length:count}, (_,i) => ({
      day_number: i+1,
      day_name: DAY_NAMES[i] || `${i+1} diena`,
      exercises: [],
    })));
    setActiveDay(0);
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      // 2. Sukurti dienas ir pratimus
      for (const day of days) {
        const dayRec = await pb.collection("workout_plan_days").create({
          plan_id:    plan.id,
      ...
      console.error(e);
      alert("Klaida išsaugant planą");
    }
    setSaving(false);
  }

  const inp = { padding:"11px 14px", borderRadius:12, border:"1.5px solid rgba(255,255,255,0.2)", background:"rgba(255,255,255,0.07)", color:"#fff", fontSize:14, fontFamily:"inherit", outline:"none", width:"100%", boxSizing:"border-box" };
      ...
      <div style={{ maxWidth:480, margin:"0 auto", padding:"16px" }}>
        ...
        {step === 2 && (
          <div>
            ...
            <div style={{ display:"flex", gap:6, overflowX:"auto", marginBottom:16, paddingBottom:4 }}>
              {days.map((d,i) => (
                <button key={i} onClick={()=>setActiveDay(i)} style={{ padding:"8px 14px", borderRadius:20, border:"none", background:activeDay===i?"#AD1457":"rgba(255,255,255,0.12)", color:"#fff", fontSize:12, fontWeight:700, cursor:"pointer", whiteSpace:"nowrap", fontFamily:"inherit", flexShrink:0 }}>
                  {d.day_name}
                  {d.exercises.length > 0 && <span style={{ marginLeft:5, fontSize:10, opacity:0.7 }}>({d.exercises.length})</span>}
                </button>
              ))}
            </div>

            {/* Dienos pavadinimas (keičiamas) */}
            <input value={days[activeDay]?.day_name||""} onChange={e=>setDays(prev=>prev.map((d,i)=>i===activeDay?{...d,day_name:e.target.value}:d))}
              style={{ ...inp, marginBottom:14, fontSize:13, fontWeight:700 }} />
            ...
            <button onClick={()=>setShowPicker(true)} style={{ width:"100%", padding:"13px", borderRadius:14, background:"rgba(255,255,255,0.1)", border:"2px dashed rgba(255,255,255,0.25)", color:"#fff", fontSize:14, fontWeight:600, cursor:"pointer", fontFamily:"inherit", marginBottom:16 }}>
              + Pridėti pratimą
            </button>
